...
class NearDemoMainActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        if (nearMainService.attemptLogin(uri)){
            supportFragmentManager.beginTransaction().replace(R.id.demo_fragment_container, transactionFragment).commit()
        }
    }
    ...
    private fun callFunction(): StringBuilder {
        ...
        if(balanceOfResponse.error == null) {
            ...
            Log.e("NearService", "$contractName.balanceOf($balanceOfArgs) ${balanceOfResponse.error?.message}")
        ...
        if(totalSupplyResponse.error == null) {
            ...
            Log.e("NearService", "$contractName.totalSupply($totalSupplyArgs) ${totalSupplyResponse.error?.message}")
        ...
        if(transferFromResponse.error == null) {
            ...
            Log.e("NearService", "$contractName.transferFrom($transferFromArgs) ${transferFromResponse.error?.message}")
        ...
        if(callViewFunctionTransactionResponse.result.status.Failure == null) {
            Log.i("NearService.", "Hash: ${callViewFunctionTransactionResponse.result.transaction.hash}")
        ...
        if(errorResponse.result.status.Failure != null) {
            Log.e("NearService.", "Error: ${errorResponse.result.status.Failure!!.ActionError!!.kind.FunctionCallError!!.ExecutionError}")
